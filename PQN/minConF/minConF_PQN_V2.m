function [x,Timespent, fobj, funEvals] = minConf_PQN_V2(funObj,x,funProj, options)
...
[verbose,numDiff,optTol,maxIter,maxProject,suffDec,corrections,adjustStep,bbInit,SPGoptTol,SPGiters,SPGtestOpt] = ...
    myProcessOptions(...
    options,'verbose',2,'numDiff',0,'optTol',1e-16,'maxIter',500,'maxProject',100000,'suffDec',1e-4,...
    'corrections',10,'adjustStep',0,'bbInit',0,'SPGoptTol',1e-6,'SPGiters',10,'SPGtestOpt',1);
...
% Output Log
if verbose >= 2
    fprintf('%10s %10s %10s %15s %15s %15s %15s\n','Iteration','FunEvals','Projections','Step Length','Function Val','Opt Cond','gap');
end
...
% Project initial parameter vector% Set up Objective Function
...
while funEvals <= maxIter
    ...
    if i == 1
        ...
        Timespent = [Timespent cputime - tStart];
        funEvals = funEvals+1;
        ...
        % Output Log
        if verbose >= 2
            fprintf('%10d %10d %10d %15.5e %15.5e %15.5e %15.5e\n',i,funEvals*funEvalMultiplier,projects,t,f,optCond,err);
        end
